...
class Code2TestPrepareToInput(Code2TestPrepareDataset):
    '''
    Дочерний класс для дополнительной подготовки датасета 
    '''
    def __init__(self, json_input_path):
    ...
    @staticmethod
    def clean_from_comments(row, col_name):
        cleaned_code = cleaned_code = re.sub(r'\s*#.*', '', row[col_name])
        return cleaned_code
    ...
    def prepare_dataset(self):
        ...
        self.code_dataset['focal_method'] = self.code_dataset.progress_apply(self.clean_from_comments, axis=1, args=('focal_method',))
        ...
        self.code_dataset['focal_method_info'] = self.code_dataset.progress_apply(self.strip_text, axis=1, args=('focal_method_info',))
        ...
        self.code_dataset['focal_cls'] = self.code_dataset.progress_apply(self.clean_from_comments, axis=1, args=('focal_cls',))
        ...
        self.code_dataset['focal_cls_info'] = self.code_dataset.progress_apply(self.strip_text, axis=1, args=('focal_cls_info',))
...
def final_preparations(code_dataset_copy):
    '''Функция подготавливает датасет к исходному виду'''
    code_dataset = code_dataset_copy.copy() 
